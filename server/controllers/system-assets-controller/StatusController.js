import Status from "../../models/system-assets/Status.js";

export const addStatus = async (req, res) => {
  try {
    const { status_name, parent_status, description } = req.body;

    const existStatus = await Status.findOne({ status_name, parent_status });
    if (existStatus)
      return res.status(400).json({ error: "This status already exists." });

    const newStatus = new Status({ status_name, parent_status, description });
...
export const updateStatus = async (req, res) => {
  try {
    ...
    const updatedStatus = await Status.findByIdAndUpdate(
      objectId,
      { status_name, parent_status, description },
      { new: true },
    );
