...
export const addUserLanguage = async (req, res) => {
    try {
        ...
        if (!existingLanguage) existingLanguage = await Language.create({ language: cleanLanguage, });


        const alreadyAdded = await UserLanguage.findOne({ userId, languageId: existingLanguage._id, });

        if (alreadyAdded) return res.status(409).json({ error: "Language already added" })
...
export const deleteUserLanguage = async (req, res) => {
    try {
        ...
        if (!userLanguageId || !mongoose.Types.ObjectId.isValid(userLanguageId))
            return res.status(400).json({ error: "Invalid userLanguageId" });

        const deleted = await UserLanguage.findOneAndDelete({ _id: userLanguageId, userId, });
...
export const getUserLanguages = async (req, res) => {
    try {
        const userId = await getDataFromToken(req);
        const userLanguages = await UserLanguage.find({ userId }).sort({ createdAt: -1 });
