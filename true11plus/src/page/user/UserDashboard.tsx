...
import { overviewStats, topMatchesData, deadlineData } from "../../data/mentor/overview";

const Overview = () => {
  // 1. Defined the User Data
  const userData = {
    id: 1,
    name: 'Rahul Sharma',
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Side - Updated with User Data */}
        <div className="lg:col-span-2 bg-(--purple) rounded-xl p-8 text-white shadow-xs hover:shadow-sm relative overflow-hidden flex flex-col justify-center">
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shadow-sm border-2 border-white/20 ${userData.avatarColor}`}>
                {userData.initials}
              </div>
              <div>
                <h3 className="font-bold text-2xl leading-tight">Welcome back, {userData.name}!</h3>
                <span className="text-blue-200 text-sm font-medium">{userData.course} Student</span>
              </div>
            </div>

            <p className="text-blue-100 mb-6 max-w-xl">
              Your overall profile score is <span className="font-bold text-white">{userData.progress}/100</span>. 
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <div className="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-lg text-sm font-medium border border-white/10 flex items-center">
                 Status: <span className="ml-1 text-emerald-300">{userData.status}</span>
              </div>
            </div>
          </div>
        </div>
