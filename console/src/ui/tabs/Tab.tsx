    ...
    <div className={`w-full min-w-0 space-y-6 ${className}`}>
      <div className="relative w-full bg-white border border-gray-200 rounded-lg shadow-sm group overflow-hidden">
        {showLeftArrow && (
          <button
            onClick={() => scroll("left")}
            className="absolute left-0 top-0 bottom-0 z-20 w-10 bg-linear-to-r from-white via-white to-transparent flex items-center justify-center text-gray-500 hover:text-purple-600 transition-colors"
          >
          ...
          {visibleTabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 whitespace-nowrap shrink-0 select-none ${
                  isActive
        ...
        {showRightArrow && (
          <button
            ...
            className="absolute right-0 top-0 bottom-0 z-20 w-10 bg-linear-to-l from-white via-white to-transparent flex items-center justify-center text-gray-500 hover:text-purple-600 transition-colors"
          >
