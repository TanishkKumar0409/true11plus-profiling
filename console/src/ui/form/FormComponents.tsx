  ...
  <div>
    <div className="flex justify-between items-center">
      <label htmlFor={id} className="block text-xs text-(--text-color) mb-1">
        {label}{" "}
        {note && (
          <span className="text-xs text-(--danger) font-normal ml-1">
            ({note})
          </span>
        )}
      </label>
      {max && (
        <span className="text-[10px] text-(--text-color-subtle)">
          {value?.length}/{max}
        </span>
      )}
    </div>
    <input
      ...
      text-(--text-color-emphasis) bg-transparent font-semibold"
    />
  </div>
);
  ...
  <div>
    ...
    <textarea
      ...
      className="w-full paragraph px-4 py-2 border border-(--border) rounded-custom 
                 focus:ring-1 focus:ring-(--border) focus:outline-none 
                 bg-transparent resize-none text-(--text-color-emphasis) font-semibold"
    ></textarea>
  </div>
);
