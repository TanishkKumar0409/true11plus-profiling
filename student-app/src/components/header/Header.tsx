  ...
  ${isCollapsed ? "lg:left-20" : "lg:left-64"}`}
    >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-xl transition-colors cursor-pointer group" onClick={() => setSidebarProfile(!sidebarProfile)}>
          ...
          <div className="h-10 w-10 rounded-full bg-gray-200 border-2 border-white shadow-sm overflow-hidden group-hover:border-purple-100 transition-colors">
            {authUser?.avatar?.[0] ? (
              <img
                src={`${import.meta.env.VITE_MEDIA_URL}${authUser?.avatar?.[0]
                  }`}
                alt="Profile"
