        ...
        <div>
            <div className="flex items-center justify-between px-2 py-1">
                <button className="flex-1 flex items-center justify-center gap-2 py-3 text-sm font-semibold hover:bg-gray-50 text-blue-600 rounded-lg transition-colors group">
                    <BiLike className="w-5 h-5 group-hover:scale-110 transition-transform" /> {post?.totalLikes}
                </button>
            ...
            <SharePostModal
                ...
                postUrl={post ? `${import.meta.env.VITE_FRONT_URL}/profile/${authUser?.username}/post/${post._id}` : ""}
            />
        </div>
    );
};
