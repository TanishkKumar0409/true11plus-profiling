...
export default function RelatedUsers({ user }: { user: UserProps | null }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mt-6">
            ...
            <p className="text-gray-500 text-xs mb-5 mt-1">Based on your skills and profile</p>
                ...
                {suggestions.map((person) => {
                    ...
                    >
                        <div className="flex items-start space-x-3">
                            {/* Avatar */}
                            <Link href={`/public-profile/${person._id}`} className="flex-shrink-0">
                                <img
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1">
                                    <h4 className="text-sm font-bold text-gray-900 truncate hover:text-purple-600 transition-colors">
                                        <Link href={`/public-profile/${person._id}`}>
                                            {person.name}
                                        </Link>
                                    </h4>
                                </div>
