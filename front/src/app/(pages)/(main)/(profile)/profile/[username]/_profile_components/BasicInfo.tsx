...
interface UserLanguage {
    _id?: string;
    languageId?: string; // ID reference
    language?: string;   // Fallback name or populated field
}
...
export default function BasicInfo({ user }: { user: UserProps | null }) {
    ...
    // --- 1. Fetch Global Languages (Reference List) ---
    const getAllLanguages = useCallback(async () => {
        try {
            const response = await API.get(`/languages`);
            setAllLanguages(response.data);
        } catch (error) {
            // Silent fail or low priority error for global list
    ...
    // --- 3. Fetch User Profile Data (Skills & Languages) ---
                ...
                <div className="flex flex-col items-start -mt-8">
                    <div className="flex justify-between w-full items-end">
                        ...
                        <div className="flex gap-2 mb-0">
                            <button className="flex items-center space-x-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-full text-sm font-medium transition-colors shadow-sm">
                                <BiUserPlus className="w-4 h-4" />
                                <span className="hidden sm:inline">Follow</span>
                            </button>
                            <button className="flex items-center space-x-2 px-4 py-2 border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-full text-sm font-medium transition-colors bg-white">
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {userLanguages.map((item, index) => {
                                    // Resolve name: Try ID lookup first, then fallback to item.language
                                    const resolvedName = getLanguageById(item.languageId) || item.language || "Unknown";
