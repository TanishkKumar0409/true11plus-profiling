...
export default function VerifyEmailConfirm() {
  ...
  return (
    <div className="flex flex-col items-center w-full text-center bg-(--primary-bg)">
      ...
      <p className="text-center text-(--text-color) my-3">
        {state.loading
      ...
      {!state.loading && countdown > 0 && (
        <p className="text-center text-sm text-(--gray) mb-3">
      ...
      {!state.loading && (
        <div className="flex flex-col gap-3 w-full">
          ...
          <button
            onClick={handleBack}
            disabled={isRedirecting}
            className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-md border border-(--border) text-(--text-color-emphasis) hover:bg-(--gray-subtle) transition disabled:opacity-50"
          >
