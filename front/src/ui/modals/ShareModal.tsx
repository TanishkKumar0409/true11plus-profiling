...
export default function ShareModal({
  ...
  return createPortal(
    ...
    >
      <div
        className={`bg-(--primary-bg) w-full sm:max-w-lg transition-all duration-300 transform rounded-custom p-6 shadow-custom ${
          isOpen
          ...
          <div className="bg-(--secondary-bg) p-5 rounded-custom shadow-custom">
            ...
            <div className="flex items-center gap-2 bg-(--primary-bg) rounded-custom shadow-custom p-1">
              <input
                readOnly
                value={postUrl}
                className="flex-1 bg-transparent  truncate px-2 sub-paragraph focus:outline-none"
                onFocus={(e) => e.target.select()}
              />
              <button
                onClick={handleCopy}
                className={`px-4 py-2 font-bold transition rounded-custom sub-paragraph whitespace-nowrap ${
                  copied ? "bg-(--success)! text-white" : "btn-shine"
                }`}
              >
                {copied ? (
                  <>
